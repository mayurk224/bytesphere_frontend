...
const FileDetailsModel = ({ file, onClose }) => {
  if (!file) return null; // Return nothing if no file is selected

  return (
    <div className="fixed top-0 left-0 right-0 z-50 flex items-center justify-center w-full h-full bg-gray-600 bg-opacity-50">
      <div className="relative w-full max-w-md bg-white rounded-lg shadow-lg dark:bg-gray-700">
        ...
        <div className="p-4 space-y-4">
          ...
          <p className="text-base text-gray-500 dark:text-gray-400">
            <strong>Last Modified:</strong> {file.modified_at ? new Date(file.modified_at).toLocaleString() : "N/A"}
          </p>
          <a
            href={file.file_url}
            target="_blank"
