...
const UploadMultipleFiles = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      const response = await axios.post("http://localhost:3248/api/files/upload-multiple", formData, {
        headers: {
  ...
  <div className="relative">
    <label className="flex flex-col items-center justify-center p-4 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer bg-gray-700 hover:bg-gray-800 transition-colors">
      <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-gray-100 mb-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
      </svg>
      
      <span className="text-sm text-gray-50">
        {files.length > 0 ? `${files.length} file(s) selected` : 'Drag files or click to browse'}
      </span>
  ...
  {/* Status Message */}
  {message && (
    <div className={`mt-3 p-2 text-sm text-center rounded ${message.includes('error') ? 'bg-red-100 text-red-700' : 'bg-green-100 text-green-700'}`}>
      {message}
    </div>
  )}
