  ...
  const handlePermanentDelete = async (folderPath) => {
    ...
    try {
      ...
      setFolders((prevFolders) =>
        prevFolders.filter((folder) => folder.folder_path !== folderPath)
      );
    } catch (error) {
      console.error(
          ...
          >
            <div
              className={`p-4 rounded-t-lg flex items-center justify-between ${
                folder.color ? `bg-${folder.color}-50` : "bg-gray-100"
              }`}
            >
                ...
                {isOptionsOpen === index && (
                  <div className="absolute right-0 mt-2 w-48 bg-white border rounded-md shadow-lg z-10">
                    <div className="py-1">
                      ...
                      <button
                        onClick={() =>
                          handlePermanentDelete(folder.folder_path)
                        }
