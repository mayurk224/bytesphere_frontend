...
const RecentFileTable = () => {
  const [files, setFiles] = useState([]); // Stores fetched files
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState("");
  const [token, setToken] = useState(localStorage.getItem("token") || "");
  const [selectedFiles, setSelectedFiles] = useState([]); // Tracks selected files for deletion
  ...
  return (
    <div className=" p-3 sm:p-6 bg-gray-800 shadow-lg rounded-lg">
        ...
        <div className="flex items-center space-x-2">
          ...
          {selectedFiles.length > 0 && (
            ...
            >
              ...
              <span>{deleting ? "Deleting..." : `Delete (${selectedFiles.length})`}</span>
            </button>
          )}
        </div>
      </div>
      ...
      {/* File Table - Desktop and Mobile Optimized */}
          ...
          <table className="w-full">
            <thead className="text-left border-b border-gray-700">
              <tr>
                ...
                {/* Hidden on mobile */}
                <th className="p-3 text-gray-400 font-medium hidden md:table-cell">Uploaded</th>
                {/* Hidden on mobile */}
                <th className="p-3 text-gray-400 font-medium hidden md:table-cell">Actions</th>
              </tr>
            </thead>
            <tbody>
              {files.map((file) => (
                <tr key={file.file_url} className="border-b border-gray-700 hover:bg-gray-700/50 transition-colors">
                  <td className="p-3">
                    <input
                      type="checkbox"
                      className="rounded border-gray-600 text-indigo-500 focus:ring-indigo-500"
                  ...
                  <td className="p-3">
                    <div className="flex items-center space-x-3">
                      ...
                      <div>
                        ...
                        {/* Mobile-only: Show date on smaller screens */}
                        ...
                        {/* Desktop-only: Show file type */}
                        <p className="text-gray-400 text-sm hidden md:block">{file.type}</p>
                      </div>
                    </div>
                  </td>
                  <td className="p-3 text-gray-300">{formatFileSize(file.size)}</td>
                  {/* Hidden on mobile */}
                  ...
                  <td className="p-3 hidden md:table-cell">
                    <div className="flex space-x-2">
                      <button onClick={() => setSelectedFile(file)} className="p-1 text-gray-400 hover:text-indigo-400 transition-colors">
                        <svg className="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      {!loading && !error && files.length === 0 && (
        <div className="py-10 sm:py-16 text-center">
          ...
          <h3 className="mt-2 text-lg font-medium text-gray-300">No files found</h3>
          <p className="mt-1 text-gray-500">Get started by uploading your first file</p>
          <div className="mt-6">
      ...
      {selectedFile && <FileDetailsModel file={selectedFile} onClose={() => setSelectedFile(null)} />}
    </div>
  );
};
