...
const AuthCallback = () => {
  ...
  useEffect(() => {
    ...
    if (!accessToken) {
      // Check if access token is in hash fragment (#access_token=)
    ...
    const handleAuthCallback = async () => {
      try {
        const response = await axios.post("http://localhost:3248/api/oauth/google-callback", { access_token: accessToken });
